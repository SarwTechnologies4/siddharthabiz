...
class Hoteluser extends DatabaseObject
{
    ...
    protected static $db_fields = array(
        ...
        'image', 'hotels_no', 'group_id', 'access_code', 'facebook_uid', 'facebook_accesstoken', 'facebook_tokenexpire',
        ...
        'vehicle_vendor_type', 'vehicle_company_name', 'vehicle_no', 'vehicle_company_contact_no', 'vehicle_vendor_address', 'permission', 'address','prop_id'
    );
    ...
    {
        global $db;
        $sql = "SELECT * FROM tbl_users WHERE type='hotel' and status='1' ORDER BY sortorder ASC ";
    ...
    public static function get_user_title($selId = '')
    {
        global $db;
        $sql = "SELECT * FROM tbl_ WHERE type='hotel' and status='1' AND id= ' .$selId. ' ORDER BY sortorder ASC ";
        $user = self::find_by_sql($sql);
        $result = '';
        if ($user):
            foreach ($user as $Row):
                $sel = ($selId == $Row->id) ? 'selected' : '';
                $result = '' . $Row->first_name . ' ' . $Row->middle_name . ' ' . $Row->last_name . '';
            endforeach;
        endif;
        return $result;
    }
