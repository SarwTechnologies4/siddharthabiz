...
class Reward extends DatabaseObject
{
    ...
    protected static $table_name = "tbl_reward";
    protected static $db_fields = array('id', 'slug', 'title', 'date2', 'date1', 'linksrc', 'linktype', 'status', 'sortorder', 'type', 'image', 'source', 'position', 'point', 'level', 'description');
    ...
    public static function get_allpopup($type = '')
    {
        ...
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' ORDER BY sortorder DESC LIMIT 10 ";
        return self::find_by_sql($sql);
    }


    public static function get_reward_option($slug = '')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' and level='{$slug}' ORDER BY sortorder ASC ";
    ...
    public static function find_all_by_level($slug='')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE level='{$slug}' ORDER BY sortorder ASC";
        $user = self::find_by_sql($sql);
        $result = '';
        if ($user):
            foreach ($user as $Row):
                $result .= '<option value="' . $Row->id . '" >' . $Row->title  . ' -- ('.$Row->point.' pts.)</option>';
            endforeach;
        endif;
        return $result;
    }
    ...
    {
        global $db;
        $result_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE level='$slug'");
