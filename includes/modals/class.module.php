...
class Module extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'parent_id', 'name', 'link', 'mode', 'icon_link', 'status', 'sortorder', 'added_date', 'properties', 'type');
    ...
    public static function get_id_by($mode, $user_type = 'hotel')
    {
        global $db;
        $result_array = self::find_by_sql("SELECT id FROM " . self::$table_name . " WHERE mode='{$mode}' AND type='$user_type' LIMIT 1");
        return !empty($result_array) ? array_shift($result_array) : false;
    }

    //Find all the rows in the current database table.
    public static function find_all_parent($user_type = 'hotel')
    {
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status='1' AND parent_id= '0' AND type='$user_type' ORDER BY sortorder ASC");
    }

    public static function find_child_by($parentId = '', $user_type = 'hotel')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' AND parent_id= {$parentId} AND type='$user_type' ORDER BY sortorder ASC";
        return self::find_by_sql($sql);
    }
    ...
    static function find_module_byparnt($parentId = 0, $type = 'admin')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE parent_id=$parentId AND type='" . $type . "' AND status=1 ORDER BY sortorder ASC";
        return self::find_by_sql($sql);
    }
