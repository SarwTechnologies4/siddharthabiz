...
class Package extends DatabaseObject
{
    ...
    {
        global $db;
        $sql = "SELECT id FROM " . self::$table_name . " WHERE slug='$slug' LIMIT 1";
        $query = $db->query($sql);
    ...
    public static function get_filterpkg_by($notId = '', $destId = '', $activId = '', $RegnId = '', $limit = '')
    {
        global $db;
        $con = !empty($notId) ? ' AND id<>"' . $notId . '" ' : '';
        ...
        $cond4 = !empty($RegnId) ? ' LIMIT "' . $limit . '" ' : '';
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' $con $cond $cond2 $cond3 ORDER BY sortorder ASC ";
        return self::find_by_sql($sql);
    }
