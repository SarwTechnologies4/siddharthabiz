...
class Dividend extends DatabaseObject
{
    ...
    {
        global $db;
        $sql = "SELECT hotel.id, hotel.long_name, SUM(inv.payment_amount) AS payment_amount, inv.period_fiscal, DATE_FORMAT(inv.date, '%Y') AS `date`";
    ...
    {
        global $db;
        $sql = "SELECT inv.*, shareholder.name as shareholders_name, DATE_FORMAT(inv.date, '%Y') AS `date`";
        $sql .= " FROM " . self::$table_name . " AS inv";
    ...
    // Dividend display
    public static function getDividend($page = "")
    {
        global $db;
