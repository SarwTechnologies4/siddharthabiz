<?php
class Hotelservices extends DatabaseObject {

	protected static $table_name = "tbl_services";
	protected static $db_fields = array('id', 'parent_id','hotelid','title', 'icon_class', 'image', 'status', 'sortorder', 'added_date');
	...
	static function find_all_bypandid($parent_id=0,$id=''){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE parent_id=$parent_id AND id=$id  ORDER BY sortorder ASC";
		return self::find_by_sql($sql);
	}

		//Find all by parent id the current database table.
		static function find_all_by_hotelid($hotelid=0){
			global $db;
			$sql = "SELECT * FROM ".self::$table_name." WHERE hotelid=$hotelid ORDER BY sortorder ASC";
			return self::find_by_sql($sql);
		}

	//Find all by parent id the current database table.
	static function find_by_hotelid($hotelid=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE hotelid=$hotelid  AND status=1 ORDER BY sortorder ASC";
		return self::find_by_sql($sql);
	}
	...
	{
		global $db;
		$cond = !empty($pid)?' AND parent_id='.$pid:'';
	...
	static function find_maximum_byparent($field="sortorder",$pid=""){
		global $db;
		$result = $db->query("SELECT MAX({$field}) AS maximum FROM ".self::$table_name." WHERE parent_id={$pid}");
		$return = $db->fetch_array($result);
		return ($return) ? ($return['maximum']+1) : 1 ;
	}

	// view services Front.
	static function getServices($limit=''){
		global $db;
	...
	static function get_all_byparnt($parent_id=0){
		global $db;
		$sql = "SELECT id,title FROM ".self::$table_name." WHERE parent_id=$parent_id AND status=1 ORDER BY sortorder DESC";
		return self::find_by_sql($sql);
	}
	
	// get the services name from it's id
	static function getServicesName($id=0){
		global $db;
