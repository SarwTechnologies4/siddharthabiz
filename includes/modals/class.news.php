...
class News extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'title', 'author', 'brief', 'content', 'status', 'meta_keywords', 'meta_description', 'news_date', 'archive_date', 'sortorder', 'type', 'viewcount', 'added_date', 'image', 'source','tags','gallery','popular');
    ...
    public static function get_popularpkg( $limit='')
    {
        global $db;
        //$cond = !empty($destid)?' AND destinationId='.$destid:'';
        //$cond2 = !empty($notid)?' AND id<>'.$notid:'';
        $cond3 = !empty($limit)?' LIMIT '.$limit:'';
        $sql = "SELECT * FROM ".self::$table_name." WHERE status=1  and popular=1 ORDER BY id DESC ".$cond3;
        return self::find_by_sql($sql);
    }

    public static function get_relatedpkg($notid='', $limit='')
    {
        global $db;
        //$cond = !empty($destid)?' AND destinationId='.$destid:'';
        $cond2 = !empty($notid)?' AND id<>'.$notid:'';
        $cond3 = !empty($limit)?' LIMIT '.$limit:'';
        $sql = "SELECT * FROM ".self::$table_name." WHERE status=1  $cond2 ORDER BY id DESC ".$cond3;
        return self::find_by_sql($sql);
    }

	public static function get_news_list($limit='') {
		global $db;
		$cond = !empty($limit)?' LIMIT '.$limit:'';
		$sql = "SELECT id, slug, title, author, brief, tags,image,gallery, news_date, source, viewcount FROM ".self::$table_name." WHERE status='1' ORDER BY news_date DESC ".$cond; 
