...
class Route extends DatabaseObject
{
    ...
    protected static $table_name = "tbl_route";
    protected static $db_fields = array('id', 'parent_id', 'title', 'icon_class', 'image', 'status', 'added_by', 'sortorder', 'added_date');
    ...
    static function find_route_by($parent_id = 0)
    {
        global $db;
        $sql = "SELECT id, title FROM " . self::$table_name . " WHERE parent_id=$parent_id AND status=1  ORDER BY sortorder ASC";
    ...
    // view features Front.
    static function getRoute($limit = '')
    {
        global $db;
    ...
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE parent_id=$parent_id AND status=1 ORDER BY sortorder ASC";
        return self::find_by_sql($sql);
    }

    // get the features name from it's id
    static function getRouteName($id = 0)
    {
        global $db;
