...
class Activities extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'parentOf', 'destinationId', 'title', 'title_brief', 'image', 'banner_image', 'brief', 'content', 'status', 'sortorder', 'meta_keywords', 'meta_description', 'added_date', 'added_by');
    ...
    public static function get_activity_option($selId=''){
        global $db;
        $sql = "SELECT id,title FROM ".self::$table_name." WHERE status='1' ORDER BY sortorder DESC";
        $activities = self::find_by_sql($sql);
        $result='';
        if($activities):
    ...
    public static function get_all_filter($selid = '')
    {
        global $db;
