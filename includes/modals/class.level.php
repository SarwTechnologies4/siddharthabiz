...
class Level extends DatabaseObject
{
    ...
    protected static $table_name = "tbl_level";
    protected static $db_fields = array('id', 'slug', 'title', 'date2', 'date1', 'linksrc', 'linktype', 'status', 'sortorder', 'type', 'image', 'source', 'position', 'point', 'exptime');
    ...
    public static function get_level($point = '', $count=0)
    {
        global $db;
        $today = date('Y-m-d');
        $cond = !empty($limit) ? ' LIMIT ' . $limit : '';
        if($count>0){
        $sql = "SELECT * FROM " . self::$table_name . " WHERE point>={$point} ORDER BY point ASC, date1 DESC LIMIT 1";
        }
        else{
            $sql = "SELECT * FROM " . self::$table_name . " WHERE point<={$point} ORDER BY point DESC, date1 DESC LIMIT 1";
    ...
    public static function get_reward_option($selId = '')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' ORDER BY sortorder ASC ";
        ...
        if ($user):
            foreach ($user as $Row):
                $sel = ($selId == $Row->slug) ? 'selected' : '';
                $result .= '<option value="' . $Row->slug . '" ' . $sel . '>' . $Row->title . '</option>';
            endforeach;
        endif;
        return $result;
    }
    ...
    public static function greater_level_count($point = '')
    {
        global $db;
        $sql = ("SELECT * FROM " . self::$table_name . " WHERE point>={$point}");
