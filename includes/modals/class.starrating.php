...
class Starrating extends DatabaseObject
{

    protected static $table_name = "tbl_starrating";
    protected static $db_fields = array('id', 'title', 'status', 'sortorder', 'added_date');
    ...
    public static function get_rating_option($selId = '')
    {
        global $db;
        $sql = "SELECT id,title FROM " . self::$table_name . " WHERE status='1' ORDER BY sortorder ASC";
        $rating = self::find_by_sql($sql);
        $result = '';
        if ($rating):
            foreach ($rating as $Row):
                $sel = ($selId == $Row->title) ? 'selected' : '';
                $result .= '<option value="' . $Row->title . '" ' . $sel . '>' . $Row->title . '</option>';
            endforeach;
        endif;
        return $result;
    }
