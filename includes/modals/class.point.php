...
class Point extends DatabaseObject
{
    ...
    protected static $table_name = "tbl_points";
    protected static $db_fields = array('id', 'user_id', 'actual_point', 'trans_id', 'usable_point', 'status', 'reg_date', 'ip_track', 'propertyid', 'userid', 'options', 'particulars', 'point', 'billamt', 'correction', 'pkgtype');
    ...
    {
        global $db;
        $sql = "SELECT * FROM tbl_point WHERE type='general' and status='1' ORDER BY sortorder ASC ";
    ...
    {
        global $db;
        $result_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE user_id='{$id}' ORDER BY id DESC");
        return !empty($result_array) ? $result_array : false;
    }

    
    public static function get_particulars_byid($id = '')
    {
        global $db;
        // $sql = "SELECT * FROM tbl_point WHERE type='general' and status='1' ORDER BY sortorder ASC ";
        $sql = "SELECT * FROM " . self::$table_name . " WHERE user_id='{$id}' AND correction<>1 ORDER BY id DESC ";
        ...
        if ($user):
            foreach ($user as $Row):
                // $sel = ($selId == $Row->id) ? 'selected' : '';
                $result .= '<option value="' . $Row->id . '" >' . $Row->particulars . '</option>';
            endforeach;
        endif;
    ...
    //seting correction flag to 1
    public static function correction_flag($id='',$point = 0){
        global $db;
        $sql = "UPDATE " . self::$table_name;
        $sql .= " SET correction = 1";
        $sql .= " WHERE id={$id}";
        $db->query($sql);
