...
switch($action)
{
    ...
    case "addReward":
        ...
        $pkgtype = !empty($_REQUEST['pkgtype'])?$_REQUEST['pkgtype']:0;

        
        //For addition or cash
        if($_REQUEST['payment_type'] == 1){

            $data = Pricerange::find_by_billamt($billamt,$pkgtype);
            ...
            //recording package type as 0 & 1
            $record->pkgtype = $pkgtype;
        ...
        /*point method which is not used*/
        ...
        /*prize which is converted to point or deduction*/
        if($_REQUEST['payment_type'] == 3){
            ...
            //recording package type as 3 which is -
            $record->pkgtype = 3;
    ...
    case "addCorrection":
        ...
        $new_point = $_REQUEST['newpoint'];
        $new_payment_type = $_REQUEST['payment_type'];

        $old_record = Point::find_by_id($_REQUEST['correction']);
        $old_payment_type = $old_record->status;
        $old_point = $old_record->point;
        ...
        $record->point = $new_point;
        $hoteldata = Hoteluser::find_by_id($_REQUEST['accsid']);
        $record->propertyid =$hoteldata->id;

        /*actual calculation for comparing with two*/
        
        //for case  + +
        if($old_payment_type == 1 && $new_payment_type ==1){
            //for Usable Point
            $result = Generaluser::deduct_usable_point($_REQUEST['idValue'],$old_point);
            $result = Generaluser::add_usable_point($_REQUEST['idValue'],$new_point);

            //for actual point
            $result = Generaluser::deduct_actual_point($_REQUEST['idValue'],$old_point);
            $result = Generaluser::add_actual_point($_REQUEST['idValue'],$new_point);
        }elseif($old_payment_type == 1 && $new_payment_type ==3){
            if(($user->usable_point - $old_point - $new_point) < 0){
                echo json_encode(array("action"=>"warning","message"=>"Insufficient Balance For Correction. Please Contact Admin!"));
                exit;       

            }
            //for Usable Point
            $result = Generaluser::deduct_usable_point($_REQUEST['idValue'],$old_point);
            $result = Generaluser::deduct_usable_point($_REQUEST['idValue'],$new_point);

            //for actual point
            $result = Generaluser::deduct_actual_point($_REQUEST['idValue'],$old_point);
        }elseif($old_payment_type == 3 && $new_payment_type ==1){
                        //for Usable Point
                        $result = Generaluser::add_usable_point($_REQUEST['idValue'],$old_point);
                        $result = Generaluser::add_usable_point($_REQUEST['idValue'],$new_point);
            
                        //for actual point
                        $result = Generaluser::add_actual_point($_REQUEST['idValue'],$new_point);
        }elseif($old_payment_type == 3 && $new_payment_type ==3){
            if(($user->usable_point + $old_point - $new_point) < 0){
                echo json_encode(array("action"=>"warning","message"=>"Insufficient Balance For Correction. Please Contact Admin!"));
                exit;       
        ...
        $correction = Point::correction_flag($_REQUEST['correction']);

        // append * in old particular for clear definition of which
        $record->particulars = $old_record->particulars ."*";
        $record->user_id = $_REQUEST['idValue'];

        /*setting status to crection (4)*/
        $record->status = $new_payment_type;
        $record->reg_date = registered();
