<?php 
	...
	{						
		...
		case "getMailaddress":
			...
			// For Mail Chimp
			$api = new MCAPI('c19b909530a1b66577557d0fd45357a5-us12');
			$list_id = "b03765c683";
			$getresult = $api->listSubscribe($list_id, $mail, '');
			
			if($getresult === true):
				$record->save();
				echo json_encode(array("message"=>"Thank you,You have now successfully subscribed to our newsletter.You will receive a confirmation email shortly.Please follow the link in it to confirm your subscription!"));	
			else:
				echo json_encode(array("message"=>"You're already subscribed!"));	
			endif;	
