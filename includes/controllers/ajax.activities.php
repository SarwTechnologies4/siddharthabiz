...
//			$record->destinationId = $_REQUEST['destinationId'];
            ...
            $record->title_brief = (!empty($_REQUEST['title_brief'])) ? $_REQUEST['title_brief'] : '';
...
//			$record->sortorder	= Activities::find_maximum_byparent("sortorder",$_REQUEST['parentOf']);
			...
			$record->sortorder	= Activities::find_maximum_byparent("sortorder");
...
//			reOrderSub("tbl_activities", "sortorder", "parentOf",$record->parentOf);
			...
			echo json_encode(array("action"=>"success","message"=>$message));	
			log_action("Activities [".$record->title."]".$GLOBALS['basic']['deletedSuccess'],1,6);
		break;
...
//			datatableReordering('tbl_activities', $sortIds, "sortorder", "parentOf",$posId,1);
            datatableReordering('tbl_activities', $sortIds, "sortorder", '','',1);
