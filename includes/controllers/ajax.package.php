<?php 
	...
	{						
		...
		case "add":
			...
			$chk = Package::check_slug($_REQUEST['slug']);
            if ($chk == '1') {
                echo json_encode(array("action"=>"warning","message"=>"Slug already Exists."));
                exit;
            }
...
//			$record->expert_id		= serialize($newArr);
            ...
            $record->days           = (!empty($_REQUEST['days'])) ? $_REQUEST['days'] : '';
...
//			}
            $record->mapgoogle      = ($_REQUEST['mapgoogle']) ? $_REQUEST['mapgoogle'] : '';
            $record->breif          = ($_REQUEST['breif']) ? $_REQUEST['breif'] : '';
            $record->overview       = ($_REQUEST['overview']) ? $_REQUEST['overview'] : '';
