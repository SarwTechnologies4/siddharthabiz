...
<?php
$moduleTablename  = "tbl_roomfeatures"; // Database table name
$moduleId 		  = 21;				// module id >>>>> tbl_modules
$moduleFoldername = "";		// Image folder name

if(isset($_GET['page']) && $_GET['page'] == "roomfeatures" && isset($_GET['mode']) && $_GET['mode']=="list"):
clearImages($moduleTablename, "roomfeatures");
clearImages($moduleTablename, "roomfeatures/thumbnails");
$parent_id = (isset($_REQUEST['id']) and !empty($_REQUEST['id']))?addslashes($_REQUEST['id']):0;
?>
<h3>
List Features <?php if(!empty($_REQUEST['id'])){$featuresInfo   = Roomfeatures::find_by_id($parent_id); echo "'s of : ".$featuresInfo->title;}?>
<?php if(!empty($_REQUEST['id'])){ ?> 
<a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="viewFeatureslist(0);" style="margin-left:2px;">
...
<div class="example-box">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
        ...
        <tbody>
            <?php $records = Roomfeatures::find_all_byparent($parent_id);    
            ...
            <tr id="<?php echo $record->id;?>">
                ...
                <?php 
                $image  =  $record->image;
                if(!empty($image) and file_exists(IMAGE_PATH.'features/'.$image)){?>
                <img src="<?php echo IMAGE_PATH;?>features/<?php echo $record->image;?>" alt="<?php echo $record->title;?>" width="25">
                <?php } ?>
                </td>
                <td><?php echo $record->icon_class?></td>
                </td>               

                <td class="text-center">
                <?php $countChild = Roomfeatures::getTotalChild($record->id);
                if($countChild){ ?>
...
if(isset($_GET['id']) && !empty($_GET['id'])):
    ...
    $featuresInfo   = Roomfeatures::find_by_id($featuresId);
...
<a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="viewFeatureslist(<?php echo !empty($featuresInfo->parent_id)?$featuresInfo->parent_id:0;?>);">
    <span class="glyph-icon icon-separator">
        <i class="glyph-icon icon-arrow-circle-left"></i>
    </span>
    <span class="button-content"> Back </span>
        ...
        <form action="" class="col-md-12 center-margin" id="features_frm">
            <div class="form-row">
                ...
                <div class="form-input col-md-4">
                    <?php $Parentview = !empty($featuresInfo->parent_id)?$featuresInfo->parent_id:0;
                        echo Roomfeatures::get_parentList_bylevel(1,$Parentview); ?>                
                </div>                
            </div>
            <div class="form-row">
                <div class="form-label col-md-2">
            ...
            <div class="form-row add-image">
                ...
                <div class="form-input col-md-10 uploader <?php echo !empty($featuresInfo->image)?"hide":"";?>">          
                   ...
                   <label><small>Image Dimensions (46 px X 46 px)</small></label>
                </div>                
            ...
            <div class="form-row">
                ...
                <div class="form-input col-md-6">
                    <input placeholder="Icon Name" class="col-md-6 validate[length[0,50]]" type="text" name="icon_class" id="icon_class" value="<?php echo !empty($featuresInfo->icon_class)?$featuresInfo->icon_class:"";?>">
