...
$moduleFoldername = "page";		// Image folder name

if(isset($_GET['page']) && $_GET['page'] == "page" && isset($_GET['mode']) && $_GET['mode']=="list"):	
@JsonclearImages($moduleTablename, $moduleFoldername);
@JsonclearImages($moduleTablename, $moduleFoldername."/thumbnails");
?>
<h3>
List Pages
<a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="AddNewPages();">
...
<div class="example-box">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
        ...
        <tbody>
            <?php $records = Page::find_by_sql("SELECT * FROM ".$moduleTablename." ORDER BY sortorder DESC ");	
...
if(isset($_GET['id']) && !empty($_GET['id'])):
	...
	$pageInfo   = Page::find_by_id($pageId);
	$status 		= ($pageInfo->status==1)?"checked":" ";
	$unstatus 	    = ($pageInfo->status==0)?"checked":" ";
	$homepage 		= ($pageInfo->homepage==1)?"checked":" ";
	$nothomepage 	= ($pageInfo->homepage==0)?"checked":" ";
endif;	
?>
<h3>
<?php echo (isset($_GET['id']))?'Edit Page':'Add Page';?>
<a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="viewpagelist();">
    <span class="glyph-icon icon-separator">
    	...
    	<form action="" class="col-md-12 center-margin" id="page_frm">        	
            <div class="form-row">
                ...
                <div class="form-input col-md-10">
                    <input placeholder="Page Title" class="col-md-6 validate[required,length[0,100]]" type="text" name="title" id="title" value="<?php echo !empty($pageInfo->title)?$pageInfo->title:"";?>">
                </div>                
            </div>  
                    ...
                    <div class="infobox info-bg">                                                               
                        <div class="button-group" data-toggle="buttons">
                            <span class="float-left">
                                <?php 
                                    if(file_exists(SITE_ROOT."images/page/".$recimg)):
                                        $filesize = filesize(SITE_ROOT."images/page/".$recimg);
                                        echo 'Size : '.getFileFormattedSize($filesize);
                                    endif;
                                ?>
                            </span> 
                            <a class="btn small float-right" href="javascript:void(0);" onclick="deleteSavedPagesimage(<?php echo $deleteid;?>);">
                                <i class="glyph-icon icon-trash-o"></i>
                            </a>                                                       
                        </div>
                        <img src="<?php echo IMAGE_PATH.'page/thumbnails/'.$recimg;?>"  style="width:100%"/>  
    		...
    		<div class="form-row">
            	<div class="form-label col-md-12">
                    ...
                    <textarea name="content" id="content" class="large-textarea validate[required]"><?php echo !empty($pageInfo->content)?$pageInfo->content:"";?></textarea>
            ...
            <div class="form-row <?php echo (!empty($pageInfo->meta_keywords) || !empty($pageInfo->meta_description))?'':'hide';?> metadata">   
            	<div class="col-md-6">
                	<textarea placeholder="Meta Keyword" name="meta_keywords" id="meta_keywords" class="character-keyword validate[required]"><?php echo !empty($pageInfo->meta_keywords)?$pageInfo->meta_keywords:"";?></textarea>
                    <div class="keyword-remaining clear input-description">250 characters left</div>
                </div>  
                <div class="col-md-6">
                	<textarea placeholder="Meta Description" name="meta_description" id="meta_description" class="character-description validate[required]"><?php echo !empty($pageInfo->meta_description)?$pageInfo->meta_description:"";?></textarea>
            ...
            <input myaction='0' type="hidden" name="idValue" id="idValue"  value="<?php echo !empty($pageInfo->id)?$pageInfo->id:0;?>" />
         </form>    
    </div>
</div>   
<script>
