...
if (isset($_GET['page']) && $_GET['page'] == "event" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    clearImages($tablename, 'event');
    clearImages($tablename, 'event/thumbnails');
    ?>
    <h3>
        Manage Event<?php echo "'s of " . $hotel_detail->title; ?>
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="addNewEvent();">
    ...
    <div class="example-box">
        <div class="example-code">
            <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                ...
                <tbody>
                <?php $records = Event::find_by_sql("SELECT * FROM " . $tablename . " WHERE 1=1 and hotel_id='" . $user_hotel_id . "' ORDER BY sortorder DESC ");
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "form"):
    if (isset($_GET['id']) and !empty($_GET['id'])):
        ...
        $rowInfo    = Event::find_by_id($rowId);
    ...
    ?>
    <h3>
        <?php echo (isset($_GET['id'])) ? 'Edit Event' : 'Add Event'; ?><?php echo " [" . $hotel_detail->title . "]"; ?>
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="viewEventList();">
    ...
    <div class="example-box">
        <div class="example-code">
            <form action="" method="post" class="col-md-12 center-margin" id="event_frm">
                ...
                <div class="form-row">
                    ...
                    <div class="form-input col-md-6">
                        <select data-placeholder="Event Category" class="chosen-selec validate[required,length[0,500]]" id="category_id" name="category_id">
                            <option value="">Choose</option>
                            <?php
                            $evntCats = EventCategory::find_all();
                            if ($evntCats): foreach ($evntCats as $evntRow):
                                $sel = (!empty($rowInfo->category_id) && $rowInfo->category_id == $evntRow->id) ? 'selected' : '';
                                ?>
                                <option value="<?php echo $evntRow->id; ?>" <?php echo $sel; ?>><?php echo $evntRow->title; ?></option>
                ...
                <div class="form-row">
                    ...
                    if (!empty($rowInfo->image)):
                        ...
                        <div class="col-md-3" id="removeSavedimg001">
                            <?php
                            if (file_exists(SITE_ROOT . "images/event/" . $imageRow2)):?>
                                <div class="infobox info-bg">
                                    <div class="button-group" data-toggle="buttons">
                                    <span class="float-left">
                                        <?php
                                        $filesize = filesize(SITE_ROOT . "images/event/" . $imageRow2);
                                    ...
                                    <img src="<?php echo IMAGE_PATH . 'event/thumbnails/' . $imageRow2; ?>" style="width:100%"/>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#image_upload').uploadify({
                ...
                'formData':     {PROJECT: '<?php echo SITE_FOLDER;?>', targetFolder: 'images/event/', thumb_width: 380, thumb_height: 478},
                'method':       'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/event/uploaded_image.php', {imagefile: filename}, function (msg) {
