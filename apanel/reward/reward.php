...
<?php
$moduleTablename = "tbl_reward"; // Database table name
$moduleId = 32;             // module id >>>>> tbl_modules
$moduleFoldername = "popup";        // Image folder name

if (isset($_GET['page']) && $_GET['page'] == "reward" && isset($_GET['mode']) && $_GET['mode'] == "list"):
        ...
        <div class="example-code">
            <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                ...
                <tr>
                    ...
                    <th class="text-left" width="5%">Points</th>

                    <th class="text-center" width="10%"><?php echo $GLOBALS['basic']['action']; ?></th>
                </tr>
                </thead>
                <tbody>
                <?php $records = Reward::find_by_sql("SELECT * FROM " . $moduleTablename . " ORDER BY sortorder DESC ");
                foreach ($records as $key => $record): ?>
                    <tr id="<?php echo $record->id; ?>">
                        ...
                        <td>
                            <div class="">
                                <?php 
                                echo !empty($record->description)?$record->description:''; ?>
                            </div>
                        </td>

                        <td>
                            <div class="">
                                <?php $t = Level::find_by_slug($record->level);
                                echo !empty($t->title)?$t->title:''; ?>
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="">
                                <?php 
                                echo !empty($record->point)?$record->point:''; ?>
                            </div>
                        </td>
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
    if (isset($_GET['id']) && !empty($_GET['id'])):
        ...
        $popupInfo = Reward::find_by_id($popupId);
    ...
    ?>
    <h3>
        <?php echo (isset($_GET['id'])) ? 'Edit Prize' : 'Add Prize'; ?>
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);"
            ...
            <form action="" class="col-md-12 center-margin" id="popup_frm">
                <div class="form-row ">
                    ...
                    <div class="form-input col-md-6">
                        <select name="level" id="level" class="form-control validate[required]" >
                            <option value="">Choose Level</option>
                            <?php $desId = !empty($popupInfo->level) ? $popupInfo->level : '';
                            echo Level::get_reward_option($desId); ?>
                        </select>
                    </div>
                </div>
