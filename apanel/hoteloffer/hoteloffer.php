<?php
$moduleTablename  = "tbl_roomapi_offers"; // Database table name
$moduleId 		  = 57;				// module id >>>>> tbl_modules
$moduleFoldername = "hoteloffer";		// Image folder name
	
if(isset($_GET['page']) && $_GET['page'] == "hoteloffer" && isset($_GET['mode']) && $_GET['mode']=="list"):       
?>
<h3>Hotel Offer Lists</h3>
...
<div class="example-box">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
        ...
        <tbody>
            <?php $records = Roomoffers::find_by_sql("SELECT * FROM ".$moduleTablename." ORDER BY sortorder DESC "); 
                  foreach($records as $key=>$record): ?>    
            <tr id="<?php echo $record->id;?>">
                <td class="text-center"><?php echo $key+1;?></td>
                <td><?php echo Hotelapi::field_by_id($record->hotel_id, 'title');?></td>
                <td><?php echo $record->title;?></td>
                <td><?php echo $record->date_from;?></td>
                <td><?php echo $record->date_to;?></td>
                <td><?php echo $record->discount;?> %</td>
                <td><?php $apply_for  =  set_na(ucwords($record->apply_for));
                       $apply_for  =  str_replace("_", " ", $apply_for);
                       echo $apply_for;
                ?></td>
                <td><?php $apply_id  =  $record->apply_id;
                   if(empty($apply_id)) echo "All Room";
                   else{
                        if($record->apply_for=='room_type'){
                             $row  = Roomtype::find_by_id($apply_id); echo $row->title;
                        }else{
                             $row  = Roomapi::find_by_id($apply_id); echo $row->title;
                        }
                   }
                ?></td>
...
?>
<h3>
<?php echo (isset($_GET['id']))?'Edit Offers':'Add Offers';?> [<?php echo Hotelapi::field_by_id($_GET['id'], 'title');?>]
      ...
      <form action="" class="col-md-12 center-margin" id="offers_frm">
          <div class="form-row">
            ...
            <div class="form-row">
                ...
                <div class="form-input col-md-10">
                    <input placeholder="From Date" class="col-md-4 validate[required]" type="text" name="date_from" id="date_from" value="<?php echo !empty($advInfo->date_from)?$advInfo->date_from:"";?>">
                </div>                
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-input col-md-10">
                    <input placeholder="To Date" class="col-md-4 validate[required]" type="text" name="date_to" id="date_to" value="<?php echo !empty($advInfo->date_to)?$advInfo->date_to:"";?>">
                </div>                
            </div>  
            ...
            <div class="form-row">
                ...
                <div class="form-checkbox-radio col-md-9">
                   <label><input type="radio" class="custom-radio apply_for"  value="all"        name="apply_for" checked="checked" <?php echo (@$advInfo->apply_for=='all') ? "checked":""; ?> ?> All Room</label>
                   <label><input type="radio" class="custom-radio apply_for"  value="room_type" name="apply_for" <?php echo (@$advInfo->apply_for=='room_type') ? "checked":""; ?> > Rooms Types</label>
                   <label><input type="radio" class="custom-radio apply_for"  value="room"      name="apply_for" <?php echo (@$advInfo->apply_for=='room') ? "checked":""; ?> > Rooms</label>
                </div>                
            </div>
