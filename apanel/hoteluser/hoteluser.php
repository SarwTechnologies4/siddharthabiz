...
if (isset($_GET['page']) && $_GET['page'] == "hoteluser" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    ?>
    <h3>
        Property Users
    ...
    <div class="example-box">
        <div class="example-code">
            <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                ...
                <tbody>
                <?php $records = Hoteluser::find_by_sql("SELECT * FROM " . $moduleTablename . " WHERE type='hotel' ORDER BY sortorder DESC ");
                foreach ($records as $key => $record): $user_id = $record->id; ?>
                    <tr id="<?php echo $record->id; ?>">
                        ...
                        <!-- <td>Username : <?php echo $record->username; ?><br>
                            Email : <?php echo !empty($record->email) ? $record->email : ''; ?><br>
                            Contact No :<?php echo !empty($record->contact) ? $record->contact : ''; ?>
                        </td> -->

                        <td>
                            <?php $result = $db->query("SELECT * FROM  tbl_apihotel  WHERE user_id='$user_id' ORDER BY id ASC");
                            $num_hotel = $db->num_rows($result);
                            if ($num_hotel > 0) {
                                $sn_hotel = 1;
                                while ($row = $db->fetch_array($result)) {
                                    ...
                                    ?>
                                    <?php echo $sn_hotel; ?>. <a
                                            href="<?php echo ADMIN_URL ?>hotelapi/profile/<?php echo $row['code'] ?>"><?php echo $row['title']; ?></a>
                                    <br>
                                    <?php
                                    $sn_hotel++;
                                }
                            ...
                            ?>
                            <a href="<?php echo ADMIN_URL ?>hotelapi/list/<?php echo $record->accesskey ?>" title="List All Hotel"
                               class="btn small bg-blue-alt tooltip-button"><i class="glyph-icon icon-align-justify"></i></a> <a
                                    href="<?php echo ADMIN_URL ?>hotelapi/addEdit/<?php echo $record->accesskey ?>" title="Add New Hotel"
                                    class="btn small bg-blue-alt tooltip-button"><i class="glyph-icon icon-plus"></i></a>
                        </td>
                        ...
                        <td class="text-center">
                            ...
                            <?php endif; ?>
                            <!-- <a href="javascript:void(0);" class="loadingbar-demo btn small bg-green tooltip-button" data-placement="top" title="Permissioin"
                            ...
                            <?php if ($user_type == 'admin') {
                                if ($record->for_package == 1000) {
                                    $statusPackageImage = ($record->package_status == 1) ? "bg-green" : "bg-red";
                                    $statusText = ($record->package_status == 1) ? $GLOBALS['basic']['clickUnpub'] : $GLOBALS['basic']['clickPub'];
                                    ?>
                                    <br>
                                    <a href="javascript:void(0);"
                                       class="btn small mrg5T <?php echo $statusPackageImage; ?> tooltip-button pstatusToggler"
                                       data-placement="top"
                                       title="<?php echo $statusText; ?>" status="<?php echo $record->status; ?>"
                                       id="imgHolder_<?php echo $record->id; ?>" moduleId="<?php echo $record->id; ?>">
                                        Verify for Package
                                ...
                                <?php }
                                if ($record->for_vehicle == 10000) {
                                    $statusPackageImage = ($record->vehicle_status == 1) ? "bg-green" : "bg-red";
                                    $statusText = ($record->vehicle_status == 1) ? $GLOBALS['basic']['clickUnpub'] : $GLOBALS['basic']['clickPub'];
                                    ?>
                                    <br>
                                    <a href="javascript:void(0);"
                                       class="btn small mrg5T <?php echo $statusPackageImage; ?> tooltip-button vstatusToggler"
                                       data-placement="top"
                                       title="<?php echo $statusText; ?>" status="<?php echo $record->status; ?>"
                                       id="imgHolder_<?php echo $record->id; ?>" moduleId="<?php echo $record->id; ?>">
                                        Verify for Vehicle
                                ...
                                <?php } ?>
                                <br><br><a style="padding: 2px 4px;" class="btn small bg-blue-alt tooltip-button"
                                           href="<?php echo ADMIN_URL ?>switch/<?php echo $record->accesskey; ?>"> <i class="glyph-icon icon-key"></i>
                                    Login</a>
                            <?php } ?>
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
    if (isset($_GET['id']) && !empty($_GET['id'])):
        ...
        $usersInfo = Hoteluser::find_by_id($userId);
            ...
            <form action="" class="col-md-10 center-margin" id="adminusersetting_frm">
                <div class="form-row hide">
                    ...
                    <div class="form-input col-md-4" style="padding:0px !important;">
                        ...
                        <input type="hidden" name="field_type" name="field_type" value="2">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label col-md-2">
                    ...
                    <div class="form-input col-md-10">
                        <input placeholder="First Name" class="col-md-4 validate[required,length[0,50]]" type="text" name="first_name" id="first_name"
                ...
                <div class="form-row">
                    ...
                    <div class="form-input col-md-10">
                        <input placeholder="Username" class="col-md-4 validate[required,maxSize[10],custom[onlyLetterNumber]]" type="text"
                               name="username" id="username" value="<?php echo !empty($usersInfo->username) ? $usersInfo->username : ""; ?>">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-label col-md-2">
                    ...
                    <div class="form-input col-md-10">
                        <input placeholder="Password" class="col-md-4 <?php echo !empty($usersInfo) ? '' : 'validate[required,length[0,50]]'; ?>"
                ...
                <div class="form-row">
                    ...
                    <div class="form-input col-md-10">
                        <input placeholder="Contact No" class="col-md-4 validate[required,length[0,50]]" type="text" name="contact" id="contact"
                ...
                <div class="form-row">
                    ...
                    <div class="form-input col-md-10">
                        <input placeholder="Email Address" class="col-md-4 validate[required,custom[email]]" type="text" id="email" name="email"
            ...
            <form action="" method="post" class="col-md-12 center-margin" id="permission_frm">
                <ul>
                    ...
                    foreach ($parentmenu as $key => $val) {
                        $childmenu = Module::find_child_by($val->id,'hotel');
                        $res .= '<li>
