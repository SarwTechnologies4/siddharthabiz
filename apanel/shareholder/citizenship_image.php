...
if (!empty($imageNameArr)):
    foreach ($imageNameArr as $key => $val):?>
        <div class="form-row">
            <div id="previewUserimage<?php echo $deleteid; ?>">
                <div class="infobox info-bg">
                    <img src="<?php echo IMAGE_PATH . 'shareholder/citizenship_image/thumbnails/' . $val; ?>"
                         style="width:100%"/>
                    ...
                    <input type="hidden" name="imageArrayname1" value="<?php echo $val; ?>"
...
if (isset($_SESSION['imageNameArr'])) {
    if (count($_SESSION['imageNameArr']) > 0) {
        foreach ($_SESSION['imageNameArr'] as $key => $val) {
            @unlink(IMAGE_PATH . 'shareholder/citizenship_image/thumbnails/' . $val);
            @unlink(IMAGE_PATH . 'shareholder/citizenship_image/' . $val);
        }
