jQuery(function($){
    ...
    $(".anim ").find('input, select').focusin(function(){
      
       $.each($(this), function(i, v){
        if( !$(v).val() ) {
            $(v).parent('.anim').addClass('min');
            }
       })
        
    });

    $(".anim").find('input, select').focusout(function(){
        $.each($(this), function(i, v){
            if( !$(v).val() ) {
                $(v).parent('.anim').removeClass('min');
                }
           })
    });
...
$(document).ready(function () {
    ...
    $("#main-register-form2").validate({
        ...
        submitHandler: function (form) {
            if (form) {
                var Frmval = $("#main-register-form2").serialize();
