...
function feedbackReply(Re)
{
	...
	function create_editor_1(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_1');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['nearby_attractions'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['nearby_attractions'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_2(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_2');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['imp_info'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['imp_info'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_3(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_3');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['restaurant'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['restaurant'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_4(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_4');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['weddinghall'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['weddinghall'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_5(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_5');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['cuisine'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['cuisine'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_6(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_6');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['breads'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['breads'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_7(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_7');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['cakes'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['cakes'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_8(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_8');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['beverages'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['beverages'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    }
	function create_editor_9(base_url, editor_arr) {
        if (editor_arr.length > 0) {
            for (var i in editor_arr) {
                CKEDITOR.replace(editor_arr[i], {filebrowserBrowseUrl: base_url + 'ckfinder/ckfinder.html'});
                CKEDITOR.dtd.$removeEmpty.i = 0;
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore_9');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['whyus'].getData();
                    if (data.match(/<hr id="system_readmore" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['whyus'].insertHtml('<hr id="system_readmore" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
