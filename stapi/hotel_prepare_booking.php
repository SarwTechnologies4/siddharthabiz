...
$api_baseurl_api  =  $api_baseurl.'api/';
...
foreach($no_of_room as $key=>$val): if(!empty($val)){ ?>
    ...
    <div class="row">
        <div class="form-group">
            <div class="col-sm-12">  
                <p>
                    <!-- <label><input type='radio'  name="payment_method" value="stripe" checked="checked"> <i class="fa fa-cc-paypal fa-3x" aria-hidden="true"></i> <i class="fa fa-cc-mastercard fa-3x" aria-hidden="true"></i> <i class="fa fa-cc-visa fa-3x" aria-hidden="true"></i> <i class="fa fa-credit-card fa-3x" aria-hidden="true"></i> </label> -->
		<?php $rows = Hotelapi::find_by_code($code); 
                ...
                <?php } else { ?> <style>img.hide-radio{ left: 5px; }</style> <?php } ?>
                    <label><input type='radio'  name="payment_method" value="pay_later" checked="checked"> <img class="img-responsive hide-radio" src="<?php echo $api_baseurl;?>hblapi/images/inquiry.png" width="210px" style="position: relative;float: right;width: 170px; top: -10px;"></label>
                </p>
		</p>
	    </div>
        </div>
