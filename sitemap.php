...
$staticblocks = array('home', 'contact', 'meetingandevents', 'offers', 'blog', 'faq');
...
$pages = Hotelapi::find_all_active();
foreach ($pages as $page) {
    $xml .= '
        <url>
            <loc>' . BASE_URL . 'hotel/' . $page->slug . '</loc>
            <lastmod>' . date('c', strtotime($page->added_date)) . '</lastmod>
            <priority>0.75</priority>
        </url>
    ';
...
$pages = Blog::find_all_active();
foreach ($pages as $page) {
    $xml .= '
        <url>
            <loc>' . BASE_URL . 'blog/' . $page->slug . '</loc>
            <lastmod>' . date('c', strtotime($page->added_date)) . '</lastmod>
