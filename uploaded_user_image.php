        ...
        <div class="form-row">
            <div class="" id="previewUserimage<?php echo $deleteid; ?>">
                <div class="infobox info-bg">
                    <img src="<?php echo IMAGE_PATH . 'user/thumbnails/' . $val; ?>" style="width:100%"/>
                    <a href="javascript:void(0);" onclick="deleteTempimage(<?php echo $deleteid; ?>);">
                        deletetemp<span class="badge badge-absolute float-right bg-red" style="right: -10px !important;">
                            <i class="glyph-icon icon-clock-os"></i>
                        </span>
                    </a>
                    <input  name="imageArrayname" value="<?php echo $val; ?>" class="validate[required,length[0,250]]"/>
                </div>
            </div>
        </div>
    <?php endforeach;
...
if (isset($_SESSION['imageNameArr'])) {
    if (count($_SESSION['imageNameArr']) > 0) {
        foreach ($_SESSION['imageNameArr'] as $key => $val) {
            @unlink(IMAGE_PATH . 'user/thumbnails/' . $val);
            @unlink(IMAGE_PATH . 'user/' . $val);
        }
